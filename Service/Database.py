...
class Database:
    ...
    @staticmethod
    def getUser(userID, withPasswd=False) -> Optional[dict]:
        """
        根据用户ID获取用户信息
    ...
    def getGroupDiscussionReplies(discussionID: int) -> list[dict]:
        ...
        reply = GroupDiscussionReply.query.filter_by(discussionID=discussionID).order_by(
            GroupDiscussionReply.replyTime.desc()).all()
    ...
    @staticmethod
    def getAllUnreadMessage(userID: int) -> dict[str, list[dict]]:
        """
        获取用户的所有未读消息
